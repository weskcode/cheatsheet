...
`app-store-aso/scripts/validate_metadata.py` — see the count next to each
...
| Secondary Category | **Developer Tools** (recommended — confirm it's still in Apple's current category list at submission time; not verified live) |
...
user's content — there is nothing in the app that scores above 4+ on any
axis. If Apple's expanded age-rating questionnaire (13+/16+/18+ categories,
required since Jan 31 2026) asks about data-linked advertising or profiling:
answer **No** to all — `PrivacyInfo.xcprivacy` declares zero data collection.
...
## 5. Promotional Text (148/170 characters)

Editable any time without a new build/review — update seasonally if useful.
...
## 6. Description (1,348/4,000 characters)
...
| Marketing URL | `https://weskcode.github.io/cheatsheet/` (optional; hosted landing page) |
| Privacy Policy URL | `https://weskcode.github.io/cheatsheet/privacy.html` (live, hosted) |

The Marketing and Privacy pages are served via GitHub Pages from the
`gh-pages` branch (static HTML, mirrors `PRIVACY.md`). If you later stand up
the AppHQ page at `https://apphq.online/apps/2635`, either point the Marketing
URL there instead or link the two — the Privacy Policy URL should stay on a
page whose text matches the App Privacy answers.
...
- **Apple's "App of the Day" / featuring nomination** — App Store Connect →
  ...
  takes two minutes, no downside to submitting it.
...
- ✅ **Privacy Policy URL** — RESOLVED. Live at
  `https://weskcode.github.io/cheatsheet/privacy.html` via GitHub Pages
  (`gh-pages` branch). Text mirrors `PRIVACY.md`.
